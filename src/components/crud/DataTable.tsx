        ...
        <div className="flex items-center gap-3">
          <div className="relative flex-1 max-w-sm">
            ...
            <input
              ...
              className="h-10 w-full rounded-xl bg-card border border-input pl-10 pr-4 text-sm text-foreground placeholder:text-muted-foreground/50 focus:outline-none focus:border-ea-green focus:ring-4 focus:ring-ea-green/10 transition-all duration-200"
            />
          </div>
              ...
              {data.length === 0 ? (
                <tr>
                  <td colSpan={columns.length + (showSelect ? 1 : 0) + (renderActions ? 1 : 0)}
                    className="px-4 py-16 text-center text-sm text-muted-foreground/60 font-mono">{emptyMessage}</td>
                </tr>
              ) : (
                data.map((row, i) => {
                  const id = getId?.(row);
                  return (
                    <tr key={id ?? i}
                      className={cn(
                        ...
                        selectedIds?.includes(id ?? '') && 'bg-ea-green/5'
                      )}>
                      {showSelect && (
