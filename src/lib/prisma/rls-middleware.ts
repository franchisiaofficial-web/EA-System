...
function createRlsPrisma() {
  ...
  if (!url)
    throw new Error('DIRECT_URL required for RLS transaction operations');
  return new PrismaClient({
    adapter: new PrismaPg({
      connectionString: url,
      max: Number(process.env.PRISMA_POOL_MAX) || 6,
    }),
  });
}
...
export async function withRls<T>(
  ...
  return rlsPrisma.$transaction(
    async (tx) => {
      ...
      if (ctx.userId)
        pairs.push({ key: 'app.current_user_id', value: ctx.userId });
      if (ctx.schoolId)
        pairs.push({ key: 'app.current_school_id', value: ctx.schoolId });
      if (ctx.membershipId)
        pairs.push({
          key: 'app.current_membership_id',
          value: ctx.membershipId,
        });
      ...
      if (pairs.length > 0) {
        const sql =
          'SELECT ' +
          pairs
            .map((_, i) => `set_config('${pairs[i].key}', $${i + 1}, true)`)
            .join(', ');
