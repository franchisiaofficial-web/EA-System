  ...
  try {
    ...
    const result = await runSimpleMutation<typeof parsed, Term>({
      ...
      execute: async (data, { authCtx: ac, requestCtx: rc }) => {
        return withRls(rc, async (tx) => tx.term.create({
  ...
  try {
    ...
    const result = await runSimpleMutation<typeof parsed, Term>({
      ...
      execute: async (data, { requestCtx: rc }) => {
        return withRls(rc, async (tx) => tx.term.update({ where: { id }, data: { ...data, ...(data.startDate ? { startDate: new Date(data.startDate) } : {}), ...(data.endDate ? { endDate: new Date(data.endDate) } : {}) } }));
      },
      getEntityId: () => id,
  ...
  try {
    ...
    const result = await runSimpleMutation<string, Term>({
      ...
      execute: async (entityId, { requestCtx: rc }) => {
        return withRls(rc, async (tx) => tx.term.update({ where: { id: entityId }, data: { status: 'INACTIVE' } }));
      },
      getEntityId: () => id,
