...
interface Section { id: string; name: string; description: string | null; status: string; }

export function SectionList({ canCreate, canUpdate, canArchive }: { canCreate: boolean; canUpdate: boolean; canArchive: boolean }) {
  ...
  return (
    <div className="space-y-6">
      ...
      <DataTable columns={columns} data={items} total={total} page={page} pageSize={10} totalPages={totalPages}
        searchPlaceholder="Search sections..." onSearch={setSearch} onPageChange={setPage}
        renderActions={(row) => (
          <div className="flex items-center gap-1 justify-end">
            {canUpdate && <Button variant="ghost" size="xs" onClick={() => router.push(`/dashboard/academics/sections/${row.id}/edit`)}><Pencil className="h-3.5 w-3.5" /></Button>}
