...
export function SectionForm() {
  ...
  const { register, handleSubmit, formState: { errors } } = useForm<FormData>({ resolver: zodResolver(schema), defaultValues: { name: '', description: '' } });
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      if (r.success) { toast.success('Created'); router.push('/dashboard/academics/sections'); router.refresh(); }
  ...
  return (
    <div className="max-w-lg space-y-6">
      <h1 className="text-2xl font-bold text-foreground">Create Section</h1>
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 rounded-xl border border-border bg-card p-6">
        <TextField label="Name" registration={register('name')} error={errors.name} required placeholder="e.g. Section A" />
