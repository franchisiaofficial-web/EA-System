...
interface Term {
  id: string; name: string; academicYearId: string; startDate: string; endDate: string; status: string;
}

export function TermList({ canCreate, canUpdate, canArchive }: { canCreate: boolean; canUpdate: boolean; canArchive: boolean }) {
  ...
  const columns: Column<Term>[] = [
    ...
    { key: 'startDate', header: 'Start', render: (r) => <span className="font-mono text-xs">{new Date(r.startDate).toLocaleDateString()}</span> },
    { key: 'endDate', header: 'End', render: (r) => <span className="font-mono text-xs">{new Date(r.endDate).toLocaleDateString()}</span> },
