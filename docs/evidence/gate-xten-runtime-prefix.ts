...
async function main() {
  console.log('=== PHASE 1.5 RUNTIME — PRE-FIX (School A admin against School B data) ===\n');

  // ── H1: exams — read foreign exam results ──
  try {
  ...
  // ── H2: exams — read foreign student results ──
  try {
  ...
  // ── H3: exams — upsert INTO foreign exam (update branch) ──
  try {
  ...
  // ── H4: fees — record payment against foreign invoice ──
  try {
  ...
  // ── H5: attendance session — close foreign session (route-equivalent query) ──
  try {
  ...
  // ── H6: attendance server-action path — client-supplied schoolId ──
  try {
  ...
  // ── M1: fees — invoice for foreign student (contamination) ──
  try {
  ...
  // ── M2: fees — fee structure for foreign class (contamination) ──
  try {
  ...
  // ── M3: promotion — foreign studentId in items (leak into failure) ──
  try {
