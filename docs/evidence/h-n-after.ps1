...
$out = 'C:\Users\BALAJI\AppData\Local\Temp\opencode\phase1.6-after.txt'
function Log($s) { Write-Output $s; Add-Content -Path $out -Value $s }

Set-Content -Path $out -Value "EA SYSTEM - PHASE 1.6 AUDIT EVIDENCE - N1-N13 AFTER FIX (identical request replays)"
...
Req 'N10a' 'POST' '/api/classes' (@{ name = 'Evidence N10'; academicYearId = 'fixture_ay_b' } | ConvertTo-Json)
...
Req 'N12a' 'POST' '/api/students' (@{ firstName = 'Evidence'; lastName = 'N12'; admissionNumber = 'EVIDENCE-N12A'; academicYearId = 'fixture_ay_b'; classId = 'fixture_cls_b_g01'; sectionId = 'fixture_sec_b_g01_a' } | ConvertTo-Json)

Section "N12b PATCH /api/students/seed_stu_001296 (in-school student, foreign refs)"
Req 'N12b' 'PATCH' '/api/students/seed_stu_001296' (@{ academicYearId = 'fixture_ay_b'; classId = 'fixture_cls_b_g01'; sectionId = 'fixture_sec_b_g01_a'; rollNumber = '99' } | ConvertTo-Json)
